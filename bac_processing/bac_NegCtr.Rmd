---
title: "FRAM OBSERVATORY -- Remote Access Samplers -- 16S rRNA amplicons (negative controls)"
---

This markdown describes the processing of negative PCR/sequencing controls from 16S rRNA amplicon-sequencing within the FRAM LTER. Raw fastq files are available under ENA umbrella PRJEB43905.
...
mkdir NegCtr
cd NegCtr
/isibhv/projects/p_bioinf2/miseq/copy-files-from-tape.sh ../files_NegCtr.txt
...
# test rename
cd Clipped
for i in *fastq.gz; do nname=`echo $i | awk '{gsub(/_L001/,"_clip");print}'`; echo -e $i $nname; done
for i in *fastq.gz; do nname=`echo $i | awk '{gsub("CTCTCTAC-CTAAGCCT_","1_");print}'`; echo -e $i $nname; done
for i in *fastq.gz; do nname=`echo $i | awk '{gsub("TCGACGTC-TCTCTCCG_","2_");print}'`; echo -e $i $nname; done
for i in *fastq.gz; do nname=`echo $i | awk '{gsub("TCGACGTC-TATCCTCT_","");print}'`; echo -e $i $nname; done
for i in *fastq.gz; do nname=`echo $i | awk '{gsub("_bactV4V5","");print}'`; echo -e $i $nname; done

# if looking OK - execute:  
for i in *fastq.gz; do nname=`echo $i | awk '{gsub(/_L001/,"_clip");print}'`; mv $i $nname; done
for i in *fastq.gz; do nname=`echo $i | awk '{gsub("CTCTCTAC-CTAAGCCT_","1_");print}'`; mv $i $nname; done
for i in *fastq.gz; do nname=`echo $i | awk '{gsub("TCGACGTC-TCTCTCCG_","2_");print}'`; mv $i $nname; done
for i in *fastq.gz; do nname=`echo $i | awk '{gsub("TCGACGTC-TATCCTCT_","");print}'`; mv $i $nname; done
for i in *fastq.gz; do nname=`echo $i | awk '{gsub("_bactV4V5","");print}'`; mv $i $nname; done
...
# setwd 
setwd("/isibhv/projects/FRAMdata/MolObs/WaterCol_RAS_ampTimeseries/bacteria/NegCtr")

# list files
path <- "/isibhv/projects/FRAMdata/MolObs/WaterCol_RAS_ampTimeseries/bacteria/NegCtr/Clipped"
fns <- list.files(path)
fns
...
dim(seqtab) # identified 83 sequences
saveRDS(seqtab, 
  "seqtab_bac_NegCtr.rds")
...
write.table(track, 
  "dadastats_bac_NegCtr.txt", 
  quote=F, sep="\t")

#################################

save.image("RAS_bac_NegCtr.Rdata")
