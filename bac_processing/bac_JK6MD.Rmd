---
title: "FRAM OBSERVATORY -- Remote Access Samplers -- 16S rRNA amplicons (run JK6MD)"
---
...
# test rename
cd Clipped
for i in *fastq.gz; do nname=`echo $i | awk '{gsub(/bact_S[0-9]{1,3}_L001/,"clip");print}'`; echo -e $i $nname; done

# if looking OK - execute:  
for i in *fastq.gz; do nname=`echo $i | awk '{gsub(/bact_S[0-9]{1,3}_L001/,"clip");print}'`; mv $i $nname; done
...
# setwd 
setwd("/isibhv/projects/FRAMdata/MolObs/WaterCol_RAS_ampTimeseries/bacteria/JK6MD")

# list files
path <- "/isibhv/projects/FRAMdata/MolObs/WaterCol_RAS_ampTimeseries/bacteria/JK6MD/Clipped"
fns <- list.files(path)
fns
...
# Save
saveRDS(seqtab, "seqtab_bac_JK6MD.rds")
...
write.table(track, 
  "dadastats_bac_JK6MD.txt", 
  quote=F, sep="\t")

#################################

save.image("RAS_bac_JK6MD.Rdata")
