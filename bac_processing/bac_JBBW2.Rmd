---
title: "FRAM OBSERVATORY -- Remote Access Samplers -- 16S rRNA amplicons (run JBBW2)"
---
...
/isibhv/projects/p_bioinf2/miseq/copy-files-from-tape.sh files_JBBW2.txt

# Rename / remove MiSeq run-ID 
rename 'M00493_511_000000000-JBBW2' 'JBBW2' *
cd JBBW2
mkdir Original 
mv *gz Original
...
# test rename
cd Clipped
for i in *fastq.gz; do nname=`echo $i | awk '{gsub(/_PS121_RAS_DNA_[0-9]{1,2}_bactV4V5_S[0-9]{1,2}_L001/,"_clip");print}'`; echo -e $i $nname; done

# if looking OK - execute:  
for i in *fastq.gz; do nname=`echo $i | awk '{gsub(/_PS121_RAS_DNA_[0-9]{1,2}_bactV4V5_S[0-9]{1,2}_L001/,"_clip");print}'`; mv $i $nname; done
...
# setwd 
setwd("/isibhv/projects/FRAMdata/MolObs/WaterCol_RAS_ampTimeseries/bacteria/JBBW2")

# list files
path <- "/isibhv/projects/FRAMdata/MolObs/WaterCol_RAS_ampTimeseries/bacteria/JBBW2/Clipped"
fns <- list.files(path)
fns
...
# Save
saveRDS(seqtab, "seqtab_bac_JBBW2.rds")
...
write.table(track, 
  "dadastats_bac_JBBW2.txt", 
  quote=F, sep="\t")

#################################

save.image("RAS_bac_JBBW2.Rdata")
