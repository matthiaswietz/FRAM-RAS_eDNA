---
title: "FRAM OBSERVATORY -- Remote Access Samplers -- 18S rRNA amplicons (run JK6MD)"
---
...
/isibhv/projects/p_bioinf2/miseq/copy-files-from-tape.sh files_JK6MD.txt

# Rename / remove MiSeq run-ID 
rename 'M03457_0078_000000000-JK6MD' 'JK6MD' *
cd JK6MD
mkdir Original 
mv *gz Original
...
# test rename
cd Clipped
for i in *fastq.gz; do nname=`echo $i | awk '{gsub(/-euk_S[0-9]{1,3}_L001/,"_clip");print}'`; echo -e $i $nname; done

# if looking OK - execute:  
for i in *fastq.gz; do nname=`echo $i | awk '{gsub(/-euk_S[0-9]{1,3}_L001/,"_clip");print}'`; mv $i $nname; done
...
# setwd 
setwd("/isibhv/projects/FRAMdata/MolObs/WaterCol_RAS_ampTimeseries/eukaryotes/JK6MD")

# list files
path <- "/isibhv/projects/FRAMdata/MolObs/WaterCol_RAS_ampTimeseries/eukaryotes/JK6MD/Clipped"
fns <- list.files(path)
fns
...
# Should retain >70% -- 0.8 here -- Good
head(out)
summary(out[, 2]/out[, 1])
...
# Save
saveRDS(seqtab, "seqtab_euk_JK6MD.rds")
...
write.table(track, 
  "dadastats_euk_JK6MD.txt", 
  quote=F, sep="\t")

#################################

save.image("RAS_euk_JK6MD.Rdata")
